...
class Orders_model extends CI_Model
{
	...
	*/
	public function getAll($status = 1, $limit = 0, $offset = 0, $sort_by, $sort_order, $search = array())
	{
		...
		$sort_columns = array('p.id', 'p.post_title', 'p.created_at', 'p.published_at');
		$sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'p.id';

		$result = array();
		$res = FALSE;

		$this->db->select('p.id, p.post_title, p.created_at, p.published_at, p.post_excerpt, u.name, p.post_status');
		$this->db->from($this->_table . ' p');
		$this->db->join('users u', 'p.post_author = u.id', 'left');

		// Status
		if ((int)$status === 1) {
			$this->db->where('p.post_status', 'initiated');
		}
		if ((int)$status === 2) {
			$this->db->where('p.post_status', 'finalized');
		}
		if ((int)$status === 3) {
			$this->db->where_in('p.post_status', array('initiated', 'finalized'));
		}

		$this->db->where('p.post_type', 'orders');
	...
	*/
	public function get($table = NULL, $fields = '', $where = array(), $order = array())
	{
		$table = (is_null($table)) ? $this->_table : $table;
...
/* Location: ./application/modules/extranet/models/orders_model.php */