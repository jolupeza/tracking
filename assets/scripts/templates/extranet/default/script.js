...
(function($){
  ...
  j(window).on('resize', function(){
    outerHtml = $html.outerHeight();

    j('.sidebar').height(outerHtml);
  });

	j(document).on("ready", function(){
    outerHeight = $header.outerHeight();
    outerBody = $body.outerHeight();
    outerHtml = $html.outerHeight();
    ...
    /*j("html").niceScroll({
      cursorcolor     : '#990000',     // change cursor color in hex, default is "#000000"
      autohidemode    : false, //how hide the scrollbar works, true=default/"cursor"=only cursor hidden/false = do not hide
