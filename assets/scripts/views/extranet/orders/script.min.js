...
h=h.split("-");h=h[h.length-1];a.push(h);});j("#dtp-orderdate").datetimepicker({defaultDate:(j("#dtp-orderdate").data("datetime"))?moment(j("#dtp-orderdate").data("datetime")):""});
j("#dtp-deliverydate").datetimepicker({defaultDate:(j("#dtp-deliverydate").data("datetime"))?moment(j("#dtp-deliverydate").data("datetime")):""});j("#js-frm-order").bootstrapValidator({feedbackIcons:{valid:"glyphicon glyphicon-ok",invalid:"glyphicon glyphicon-remove",validating:"glyphicon glyphicon-refresh"},live:"enabled",fields:{order_title:{validators:{notEmpty:{message:"Campo requerido"}}},order_detail:{validators:{notEmpty:{message:"Campo requerido"}}},order_site:{validators:{notEmpty:{message:"Campo requerido"}}},order_customer:{validators:{callback:{message:"Seleccionar cliente",callback:function(g,f){if(g=="0"){return false;
}return true;}}}},order_date:{validators:{notEmpty:{message:"Debe indicar fecha"},date:{format:"YYYY-MM-DD",message:"Fecha no válida",},callback:{message:"Indicar fecha mayor o igual a la fecha actual y menor a la fecha de entrega",callback:function(k,h){var f=new moment(k,"YYYY-MM-DD",true);
...
}}if(!f.isValid()){return false;}if(f.unix()<moment(g).unix()){return false;}return true;}}},onSuccess:function(h,g){var f=g.element[0].value;j(".main__content__observation__article").first().find('input[id^="order_obs_date"]').val(f);
j('div[id^="dtp-date-state"]').each(function(i,k){j(this).data("DateTimePicker").minDate(f);});},onError:function(g,f){j(".main__content__observation__article").first().find('input[id^="order_obs_date"]').val("");
j('div[id^="dtp-date-state"]').each(function(h,i){j(this).data("DateTimePicker").minDate(false);});}},order_deliverydate:{validators:{notEmpty:{message:"Debe indicar fecha"},date:{format:"YYYY-MM-DD",message:"Fecha no válida",},callback:{message:"Indicar fecha mayor o igual a la fecha de pedido",callback:function(k,h){var f=new moment(k,"YYYY-MM-DD",true);
