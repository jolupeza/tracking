...
}else{href_new=_root_+d[3]+"/"+d[4]+"/"+d[5]+"/"+d[6]+"/"+d[7]+"/"+d[8]+"/"+f+"/"+d[10];}spinner.stop();j(".main__grid__content").load(href_new);});j("#js-frm-add-publi").bootstrapValidator({feedbackIcons:{valid:"glyphicon glyphicon-ok",invalid:"glyphicon glyphicon-remove",validating:"glyphicon glyphicon-refresh"},live:"enabled",fields:{publi_name:{validators:{notEmpty:{message:"Campo requerido"}}},},}).on("success.form.bv",function(k){k.preventDefault();
spinner.spin(target);var i=j(this);var d=j(k.target);var c=d.serializeArray();var h=c[0].value;var g=c[1].value;var f=c[2].value;j.post(_root_+"extranet/advertising/add",{token:h,name:g,avatar:f},function(e){spinner.stop();
...
}},"json");});j("#js-frm-edit-publi").bootstrapValidator({feedbackIcons:{valid:"glyphicon glyphicon-ok",invalid:"glyphicon glyphicon-remove",validating:"glyphicon glyphicon-refresh"},live:"enabled",fields:{publi_name:{validators:{notEmpty:{message:"Campo requerido"}}},},});
